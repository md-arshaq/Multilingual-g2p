...
# ── Try adjustText; graceful fallback if not installed ────────────────────────
try:
...
# STEP 0: LOAD ALL INPUTS — derive everything from phoneme_cluster_mapping.json
...
LABEL_STYLE_MAP = [
    ...
    ("Rare",                        "#B2BEC3", "rare"),
    ("Misc",                        "#9EA7AD", "rare"),       # fallback
]

# A small palette for clusters whose label doesn't match any keyword above
FALLBACK_COLORS = [
...
# ── Cluster regions — tight std-dev ellipses (no expansion, avoids hull bleed) ─
...
for cid, phones in clusters.items():
    ...
    # 1.5 std radius — tight enough that interleaved clusters don't swamp each other
...
# Offsets are computed automatically — no hardcoded IDs
...
r = type_counts["rare"]
# Only include slices with non-zero count
...
# Build cluster table rows dynamically
cluster_table_rows = ""
